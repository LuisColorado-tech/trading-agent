...
export default async function GridStablePage() {
  const [statsRes, tradesRes] = await Promise.allSettled([
    api.gridStableStats(),
    api.gridStableTrades(),
  ])

  const st = statsRes.status === 'fulfilled' ? statsRes.value : {}
  const allTrades = tradesRes.status === 'fulfilled' ? tradesRes.value : []
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.4s_ease-out]">
      <div>
        <h1 className="text-2xl font-bold text-white tracking-tight">Grid Stable — Pares Estables</h1>
