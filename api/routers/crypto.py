...
def _get_active_crypto_session():
    return q_one("SELECT * FROM paper_sessions WHERE status='ACTIVE' ORDER BY started_at DESC LIMIT 1")
...
@router.get('/trades')
def get_trades(limit: int = 200, asset: str = None, status: str = None, scope: str = Query("all", description="all | session")):
    filters = ['1=1']
    params = {'limit': limit}
    if scope == "session":
        session = _get_active_crypto_session()
        if session:
            filters.append('timestamp_open >= :started_at')
...
def trade_stats(scope: str = Query("all", description="all | session")):
    if scope == "session":
        session = _get_active_crypto_session()
        if session:
            row = q_one("""
...
def get_trades_by_strategy(strategy: str = Query(None), limit: int = 200, scope: str = Query("all", description="all | session")):
    ...
    if scope == "session":
        session = _get_active_crypto_session()
        if session:
            session_filter = "AND timestamp_open >= :started_at"
            params['started_at'] = session['started_at']
    if strategy:
        params['s'] = strategy
        return q(f"""
            SELECT * FROM trades WHERE strategy = :s AND paper_trade = true {session_filter}
            ORDER BY timestamp_open DESC LIMIT :limit
        """, params)
    return q(f"""
        SELECT * FROM trades WHERE paper_trade = true {session_filter}
        ORDER BY timestamp_open DESC LIMIT :limit
    """, params)
    ...
    for s in strategies:
        if started_at:
            row = q_one("""
                ...
                FROM trades WHERE strategy = :s AND timestamp_open >= :started_at
            """, {'s': s, 'started_at': started_at})
        else:
            row = q_one("""
                SELECT
                    COUNT(*) FILTER (WHERE status = 'CLOSED') AS total_closed,
...
def stats_by_asset(limit: int = 20, scope: str = Query("all", description="all | session")):
